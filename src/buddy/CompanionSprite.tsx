...
import { renderFace, renderSprite, spriteFrameCount, SHIMMER_PAUSE, SHIMMER_SPEED, SHIMMER_WAIT } from './sprites.js';
...
export function CompanionSprite(): React.ReactNode {
  ...
  // Shimmer animation state for shiny companions
  const shimmerPauseTicks = SHIMMER_PAUSE / TICK_MS;
  const shimmerWaitTicks = SHIMMER_WAIT / TICK_MS;
  const totalCycleTicks = shimmerPauseTicks + shimmerWaitTicks;
  const isShimmering = companion.shiny ? tick % totalCycleTicks < shimmerPauseTicks : false;
  const charTime = tick * TICK_MS;
  ...
  // Calculate shimmer position for shiny effect
  ...
  const spriteColumn = (
    <Box flexDirection="column" flexShrink={0} alignItems="center" width={colWidth}>
      {sprite.map((line, i) => {
        // Apply shimmer effect for shiny companions
        if (companion.shiny && isShimmering) {
          const shimmerSegments = renderShimmerLine(line, i, charTime, glimmerIndex);
          return (
            <Text key={i}>
              {shimmerSegments.map((seg, j) => (
                <Text key={j} color={i === 0 && heartFrame ? 'autoAccept' : seg.color}>
                  {seg.char}
                </Text>
              ))}
            </Text>
